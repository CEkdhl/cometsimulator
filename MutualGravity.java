...
public class MutualGravity implements FirstOrderDifferentialEquations {
     ...
     * @param externalForces A list of external forces that act on each body.
     */
    public MutualGravity(ArrayList<KnownBody> knownBodies, ArrayList<UnknownBody> unknownBodies, ArrayList<ExternalForce> externalForces) {
        this.knownBodies = knownBodies;
        this.unknownBodies = unknownBodies;
        this.externalForces = externalForces;
    }

    public MutualGravity(ArrayList<KnownBody> knownBodies, ArrayList<UnknownBody> unknownBodies) {
        this.knownBodies = knownBodies;
        this.unknownBodies = unknownBodies;
    ...
    public void computeDerivatives(double t, double[] y, double[] yp) {
        ...
        for(int i=0; i<unknownBodies.size(); i++) {
            ...
            for(int k=0; k<unknownBodies.size(); k++) {
                if(k==i || unknownBodies.get(k).getMass() == 0) continue;

                double deltax = y[6*k]-y[6*i];
                double deltay = y[6*k+1]-y[6*i+1];
                double deltaz = y[6*k+2]-y[6*i+2];

                double coeff = G*unknownBodies.get(k).getMass()/pow(deltax*deltax+deltay*deltay+deltaz*deltaz,1.5);
                yp[6*i+3] += coeff*deltax;
                yp[6*i+4] += coeff*deltay;
                yp[6*i+5] += coeff*deltaz;
            }

            for(ExternalForce f: externalForces) {
                double[] state = {y[6*i], y[6*i+1],y[6*i+2], y[6*i+3], y[6*i+4], y[6*i+5], y[6*i+5]};
